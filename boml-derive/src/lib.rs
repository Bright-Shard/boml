...
use quote::quote;
use syn::{parse::Parse, parse_macro_input, Attribute, DataEnum, DataStruct, DeriveInput, FieldsNamed, FieldsUnnamed, Generics, Ident, Token, TypeParam, Variant};
...
fn derive_enum(ident: Ident, generics: Generics, attrs: Vec<Attribute>, data: DataEnum) -> Result<TokenStream, syn::Error> {
	...
	let strategy_quote = match strategy {
		EnumStrategy::ValueEnum => return Err(syn::Error::new(Span::call_site(), "value_enum is not implemented yet")),
		EnumStrategy::Untagged => return Err(syn::Error::new(Span::call_site(), "untagged is not implemented yet")),
	...
	fn try_from(value: BomlAttr) -> Result<Self, Self::Error> {
		macro_rules! mutex_error {
			($span:expr, $lhs:expr,$rhs:expr) => {
				Err(syn::Error::new($span, format!("`{}` and `{}` are mutually exclusive", $lhs, $rhs)))
			};
		}
